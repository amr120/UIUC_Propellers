...
plt.clabel(contour, inline=True, fontsize=14, fmt=lambda x: f'+{x:.0f}%' if x > 0 else f'{x:.0f}%', manual=True)
plt.clabel(contour2, inline=True, fontsize=14, fmt=lambda x: f'{x:.0f}%', manual=True)
